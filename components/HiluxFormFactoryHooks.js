...
export default function HiluxFormFactory (inputJSON, formName) {
  ...
  function addCall(bName){    
    ...
    let finalFormCore = generateCoreFromJson();
...
function staticSingleGroupView (componentBlock, componentName) {
  ...
  function addInstanceToJSON(groupName) {
    ...
    */      
    ...
    if (nameSplitArray.length == 2)
    {
      ...
      //console.log("inserted_json is : ", JSON.stringify(jsonData.fieldOrder) );
  ...
  async function formTemplate(){
    console.log("*********************start form template")
    finalCompleteForm = await fGen();

    if (finalCompleteForm != null) {
      setFormData(  finalCompleteForm)
      setIsLoading(false);
      setFormPrepInProcess(false);  
      console.log("*********************completed form template")
    }
    console.log("********************* after if*  form template")
  ...
  async function fGen() {
   ...
   async function loadServerForm(handleChange, values){
    
    setFormPrepInProcess(true);
    formDone = await getFormJSON();
    formDataDone = await getFormStaticValues();

    if (formDone && formDataDone)
    {
      //console.log( "Retrieved  ", jsonData , formContentValues.length )    
